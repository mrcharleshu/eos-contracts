namespace eosio {
    ...
    public:
        static int getDateInterval(uint64_t date1_sec, uint64_t date2_sec) {
            // FIXME
            utils handler = utils();
            return handler.getDifference(handler.secondsToDate(date1_sec), handler.secondsToDate(date2_sec));
        }
    };
}